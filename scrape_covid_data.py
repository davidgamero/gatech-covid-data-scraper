...
def parse_num_cases(row):
    ...
    # 'Students (N)' format
    re_position_students_paren = RE_POSITION_STUDENTS_PAREN.match(row_position)
...
if(path.exists(COVID_DATA_CSV)):
    os.remove(COVID_DATA_CSV)

with open(COVID_DATA_CSV, 'w') as csvfile:
